...
int comp_wildcard(const char *wildcard, const char *url){
  ...
  token_wc = strtok(wildcard_copy, separator);
  token_url = strtok(url_copy, separator);
  printf("----------------------\n");
  printf("%s %s\n",token_wc, token_url);
  ...
  while(token_wc != NULL && token_url != NULL){
      ...
      if(token_index == 0){
          ...
          */
          ...
          // else we just need to step through the index

          // compare the tokens -- if they are equal, then we do nothing, but if they are not equal then the wildcard url and the given url are not equivalent since their fields do not match
      ...
      // get the next tokens
      token_wc = strtok(NULL, separator);
      token_url = strtok(NULL, separator);
      printf("%s %s\n",token_wc, token_url);
      token_index++;
  }
...
int is_wildcard(const char *url){
  ...
  token = strtok(url_copy,separator);
...
int get_wildcard_index(const char *wildcard){
  if(is_wildcard(wildcard) == FALSE){
    // return a error if it is not a wildcard
    return -1;
  }
  ...
  token = strtok(wildcard,separator);
  // step through each token
  while(token != NULL){
      // process the first token
